        ...
        if p_sel:
            ...
            if not items_obra.empty and not df_comp.empty:
                ...
                def aplicar_redondeo(fila):
                    valor = fila['Parcial']
                    tipo = str(fila.get('Redondeo', 'Decimal')).strip().capitalize()
                ...
                # 6. BOTONES DE DESCARGA Y SUBIDA
                ...
                with col_down2:
                    ...
                    # Botón de descarga: Al hacer clic, Streamlit ejecuta la lógica de descarga
                    ...
                    # Intentar la subida automática
                    if btn_pdf:
                        exito_nube = subir_a_gcs(pdf_fp, nombre_pdf)
                        if exito_nube:
                            st.toast("☁️ Copia guardada en el historial de la nube", icon="💾")
            else:
                st.info("Este proyecto no tiene ítems con materiales asignados.")
    else:
